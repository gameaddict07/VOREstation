		...
		switch(event_type)
			...
			if(GOSSIP)
				...
				if(prob(33))
					...
					"the decision to be dropped by a major sponsor","a disasterous interview on Virgo Prime Tonight")]."
			if(TOURISM)
				...
				Virgo Prime Daily is offering discount tickets for two to see [random_name(pick(MALE,FEMALE))] live in return for eyewitness reports and up to the minute coverage."
...
/datum/event/trivial_news/announce()
	...
	"[pick("Unathi","Spacer")] gets tattoo of Virgo Prime on chest '[pick("CentCom","star","starship","asteroid")] tickles most'.",\
	...
	"Survey: 'Cheese Louise' Voted Best Pizza Restaurant In Virgo Prime",\
