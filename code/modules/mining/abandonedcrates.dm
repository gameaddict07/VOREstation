...
/obj/structure/closet/crate/secure/loot/New()
	...
	switch(loot)
		...
		if(16 to 20)
			for(var/i = 0, i < 10, i++)
				var/newmineral = pick(typesof(/obj/item/weapon/ore) - /obj/item/weapon/ore)
				new newmineral(src)
		if(21 to 25)
			for(var/i = 0, i < 3, i++)
		...
		if(61 to 62)
			for(var/i = 0, i < 12, ++i)
				new/obj/item/clothing/head/accessory/kitty(src)
		if(63 to 64)
			var/t = rand(4,7)
			for(var/i = 0, i < t, ++i)
				var/newcoin = pick(typesof(/obj/item/weapon/coin) - /obj/item/weapon/coin)
				new newcoin(src)
		if(65 to 66)
		...
		if(91)
			new/obj/item/weapon/aiModule/communist(src)
		if(92)
		...
		if(100)
			new/obj/item/mysteryBox(src) // Illegal goodies.
