...
/datum/admins/proc/Secrets(var/section)
	if(!check_rights(0))	return
	show_secrets_panel()

/datum/admins/proc/show_secrets_panel(var/section)
	if(!check_rights(0))	return
	
	var/dat = "<B>The first rule of adminbuse is: you don't talk about the adminbuse.</B><HR>"
	
	// Show the tab selection links
	if(check_rights(R_ADMIN,0))
		dat += "<a href='?src=\ref[src];showsecrets=admin'>Admin Secrets</a> "
	if(check_rights(R_FUN,0))
		dat += "<a href='?src=\ref[src];showsecrets=events'>'Random' Events</a> "
	if(check_rights(R_FUN | R_SERVER,0))	
		dat += "<a href='?src=\ref[src];showsecrets=fun'>Fun Secrets</a> "
	if(check_rights(R_SERVER | R_DEBUG,0))
		dat += "<a href='?src=\ref[src];showsecrets=useful'>Useful Secrets</a> "
	if(check_rights(R_DEBUG,0))
		dat += "<a href='?src=\ref[src];showsecrets=coder'>Coder Secrets</a> "
	dat += "<HR>";
	
	if(section == "admin")
		dat += "<B>Admin Secrets</B><BR><BR>"
		if (check_rights(R_ADMIN,0))
			dat += {"
	...
	if (section == "events")
		dat += "<B>'Random' Events</B> (Deprecated, try and use Event Manager instead if you can)<BR><BR>"
		if (check_rights(R_FUN,0))
			dat += {"
	...
	if (section == "useful")
		dat += "<B>Useful Secrets</B><BR><BR>"
		if (check_rights(R_SERVER,0))
			dat += {"
				<A href='?src=\ref[src];secretsfun=quickpower'>Charge all SMES</A><BR>
				<A href='?src=\ref[src];secretsfun=power'>Charge all APCs</A><BR>
				<A href='?src=\ref[src];secretsfun=launchshuttle'>Launch a shuttle</A> (Use This One)<BR>
	...
	var/datum/browser/popup = new(usr, "secrets", "Secrets", 550, 600)
